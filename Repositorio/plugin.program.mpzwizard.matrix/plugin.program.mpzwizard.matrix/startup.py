...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        logging.log("[Instalacion Build Check] Se extrajo la Build  {0}/100 with [ERRORS: {1}]".format(CONFIG.EXTRACT,
                                                                                                    CONFIG.EXTERROR),
                    level=xbmc.LOGINFO)
        ...
        if yes:
            ...
            logging.log("[Build Installed Check] Instalacion limpia Re-activated", level=xbmc.LOGINFO)
        else:
            logging.log("[Build Installed Check] Reinstall Ignored")
    elif CONFIG.SKIN in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
        logging.log("[Build Instalada Check] Incorrect skin: {0}".format(CONFIG.SKIN), level=xbmc.LOGINFO)
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                logging.log("[Build Instalada Check] Guifix was set to http://", level=xbmc.LOGINFO)
    ...
    if CONFIG.get_setting('installed') == 'true':
        ...
        if CONFIG.get_setting('Mantenerdebrid') == 'true':
            from resources.libs import debridit
            logging.log('[Build Installed Check] Restaurar Real Debrid Data', level=xbmc.LOGINFO)
...
# FIRST RUN SETTINGS
if CONFIG.get_setting('Primera_Instalacion') == 'true':
...
if tools.open_url(CONFIG.BUILDFILE, check=True) and CONFIG.get_setting('installed') == 'false':
    logging.log("[Build Actual Check] Build No Instalada", level=xbmc.LOGINFO)
    window.show_build_prompt()
else:
    logging.log("[Build Actual Check] Build Instalada: {0}".format(CONFIG.BUILDNAME), level=xbmc.LOGINFO)
...
if CONFIG.get_setting('installed') == 'false':
    logging.log("[Build Installed Check] Started", level=xbmc.LOGINFO)
    installed_build_check()
else:
    logging.log("[Build instalada Check] Not Enabled", level=xbmc.LOGINFO)

# SAVE TRAKT
if CONFIG.get_setting('Mantener_trakt') == 'false':
...
# SAVE DEBRID
if CONFIG.get_setting('Mantener_debrid') == 'false':
...
# SAVE LOGIN
if CONFIG.get_setting('Mantener_login') == 'false':
...
if CONFIG.get_setting('autoclean') == 'true':
    logging.log("[Limpieza automática] Started", level=xbmc.LOGINFO)
    auto_clean()
else:
    logging.log('[Limpieza automática] Not Enabled', level=xbmc.LOGINFO)

#########################################################
#         CREDITOS Y AGRADECIMIENTOS: WIZARD DE DIGGZ Y GRUPO CHIKIRY POR LA TRADUCCION        #
#########################################################